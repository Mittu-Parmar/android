...
public class CartAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        switch (cartItemModelList.get(position).getType()) {
            case CartItemModel.CART_ITEM:
                ...
                ((CartItemViewHolder) holder).setCartItemDetails(productId, resource, title, freeCoupens, productPrice, cuttedPrice, offerceApplied, position, inStock, String.valueOf(productQuantity), maxQuantity, qtyError, qtyIds, stockQty,cod);
                break;
            case CartItemModel.TOTAL_AMOUNT:
    ...
    class CartItemViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView coupenRedeemptionTextView;
...
//    =============coupen Dialog================
        ...
        public CartItemViewHolder(@NonNull View itemView) {
            ...
            this.deleteBtn = itemView.findViewById(R.id.remove_item_btn);
            ...
            this.coupenRedeemptionTextView = itemView.findViewById(R.id.coupen_redeemption_text_view);
            this.codIndicator=itemView.findViewById(R.id.cod_indicator);
        }
        ...
        void setCartItemDetails(final String productId, String productImage, String productTitle, long freeCoupens, final String productPriceText, String cuttedPrice, long offerceApplied, final int position, boolean inStock, final String quantity, final long maxQuantity, boolean qtyError, final List<String> qtyIds, final long stockQty,boolean cod) {
            ...
            if (inStock) {
                ...
                RewordsAdapter rewordsAdapter = new RewordsAdapter(position, DbQueries.rewardsModelList, true, coupensRecyclerView, selectedCoupen, productOriginalPrice, coupenTitle, coupenExpiryDate, coupenBody, discountedPrice,cartItemModelList);
                coupensRecyclerView.setAdapter(rewordsAdapter);
                rewordsAdapter.notifyDataSetChanged();

                applyCoupenButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                        ...
                        if (!TextUtils.isEmpty(cartItemModelList.get(position).getSelectedCouponId())) {
                            for (RewardsModel rewardsModel : DbQueries.rewardsModelList) {
                                if (rewardsModel.getCouponId().equals(cartItemModelList.get(position).getSelectedCouponId())) {
                                    ...
                                    coupenRedemptionLayout.setBackground(itemView.getContext().getResources().getDrawable(R.drawable.reword_gredian_background));
                                    coupenRedeemptionTextView.setText(rewardsModel.getBody());
                            ...
                            coupenApplied.setText("Coupon applied -Rs." + offerDiscountedAmount);
                ...
                removeCoupenButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        ...
                        coupenBody.setText("Tab the icon onthe right corner to select your coupon");
                        ...
                        coupenRedeemptionTextView.setText("Apply your coupon hear,");
                    ...
                    for (RewardsModel rewardsModel : DbQueries.rewardsModelList) {
                        if (rewardsModel.getCouponId().equals(cartItemModelList.get(position).getSelectedCouponId())) {
                            ...
                            if (rewardsModel.getType().equals("discount")) {
                                this.coupenTitle.setText(rewardsModel.getType());
                            } else {
                                this.coupenTitle.setText("Flat RS." + rewardsModel.getDisORamt() + " OFF");
                            }
                            final SimpleDateFormat simpleDateFormat = new SimpleDateFormat("dd MMMM YYYY");
                            this.coupenExpiryDate.setText(" Till " + simpleDateFormat.format(rewardsModel.getTimeStamp()));
                        }
                    }
                                ...
                                if (!TextUtils.isEmpty(quantityNo.getText())) {
                                    if (Long.parseLong(quantityNo.getText().toString()) <= maxQuantity && Long.parseLong(quantityNo.getText().toString()) != 0) {
                                        if (itemView.getContext() instanceof MainActivity || itemView.getContext() instanceof CartFragmentActivity) {
                                            cartItemModelList.get(position).setProductQuantity(Integer.parseInt(quantityNo.getText().toString()));
                                        } else {
                ...
                if (offerceApplied > 0) {
                    ...
                    String offerDiscountedAmount = String.valueOf(Long.valueOf(cuttedPrice) - Long.valueOf(productPriceText));
                    this.offerceApplied.setText("Offer applied -Rs." + offerDiscountedAmount);
                } else {
